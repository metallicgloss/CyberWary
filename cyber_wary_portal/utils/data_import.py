...
def search_cpe(software):
    ...
    for application in software:
        ...
        keyword = ""
        cpe = search_cpe_manual_override(application)
        ...
        if(cpe != ""):
            for word in application.name.split(" "):
                ...
                if(title_filter.count() > 1):
                    version_lookup = search_cpe_by_version(title_filter, application.version)
                    ...
                    if(version_lookup == None):
                        ...
                        cpe = search_cpe_by_remote(keyword, application.version)
                        pass
...
def search_cpe_by_remote(name, version):
    ...
    if(remote_cpe['totalResults'] > 0):
        existing_cpe = CPE.objects.filter(identifier=remote_cpe['result']['cpes'][0]['cpe23Uri'])
